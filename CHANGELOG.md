...
## [2026-02-12] v1.4.0 - 판매 집계 입력 기능 확장

### 🎉 주요 기능 추가
- **📊 복붙 기반 판매 집계**: 상품명/수량을 붙여넣어 품목별/하루 총 판매량 집계
  - 탭/콤마 구분 자동 인식
  - 다중 상품명(콤마) 입력 시 수량 분할 집계
- **집계 방식 토글**: 자동 분류/자동 집계 토글 제공
- **원문 상품명 집계**: 주문관리시트 품목별 집계를 원문 기준으로 계산 (자동 분류로 전환 가능)

### ✨ 개선 사항
- **케이블 분류 분리**: 케이블(일반)과 케이블s 분류 정확도 개선
- **택배 파일 다중 업로드**: 주문관리시트 생성 시 택배 파일 여러 개 업로드 가능

---

## [2026-02-09] v1.3.1 - 와디즈 주문관리/전화번호 서식 개선

### 🐛 수정 사항
- **와디즈 주문관리시트 반영 누락 수정**: 주문관리시트 생성 흐름에 와디즈 매핑 추가
- **전화번호 0 누락 방지**: 모든 생성 파일의 전화/연락처 컬럼을 텍스트 형식으로 저장
  - 발주 파일, 주문관리시트, 쿠팡 정렬본, 쿠팡 발송 파일 모두 적용
